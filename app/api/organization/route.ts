...
export async function POST(request: Request) {
  try {
     ...
     * --header 'Cookie: JSESSIONID=2290C560807415D82AB28C78678438C9' \
     * --data '{
     *     "username": "admin",
     *     "password": "admin",
     *     "role": "0"
    ...
    const swsToken = await fetch(`${process.env.ETENDO_URL}/sws/login`,
      {
        ...
        body: JSON.stringify({
          username: environment.name + 'Admin',
          password: 'admin',
        })
      }
    );
    ...
    formData.append('inpOrgUser', environment.name + 'User'); // Static value; adjust if dynamic
    formData.append('inpNodes', '7BFA8FF057AB46CAAB2FAAED8B870E32'); // Static or dynamic based on environment
    formData.append('inpPassword', 'admin'); // Ensure this is securely handled
    formData.append('inpConfirmPassword', 'admin'); // Ensure this is securely handled
    ...
    return new NextResponse(responseData, { status: 200, headers: { 'Content-Type': 'text/html' } });
